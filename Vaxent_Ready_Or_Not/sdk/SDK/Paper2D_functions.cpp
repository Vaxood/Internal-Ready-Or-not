...
void UPaperTileMapComponent::GetMapSize(int32* Param_MapWidth, int32* Param_MapHeight, int32* NumLayers)
{
	...
	if (Param_MapWidth != nullptr)
		*Param_MapWidth = std::move(Parms.Param_MapWidth);

	if (Param_MapHeight != nullptr)
		*Param_MapHeight = std::move(Parms.Param_MapHeight);

	if (NumLayers != nullptr)
		*NumLayers = std::move(Parms.NumLayers);
}
...
void UTileMapBlueprintLibrary::BreakTile(const struct FPaperTileInfo& Tile, int32* TileIndex, class UPaperTileSet** TileSet, bool* bFlipH, bool* bFlipV, bool* bFlipD)
{
	...
	if (TileIndex != nullptr)
		*TileIndex = std::move(Parms.TileIndex);

	if (TileSet != nullptr)
		*TileSet = std::move(Parms.TileSet);

	if (bFlipH != nullptr)
		*bFlipH = std::move(Parms.bFlipH);

	if (bFlipV != nullptr)
		*bFlipV = std::move(Parms.bFlipV);

	if (bFlipD != nullptr)
		*bFlipD = std::move(Parms.bFlipD);
}
