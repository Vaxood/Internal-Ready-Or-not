...
public:
	...
	static class FString Accessor__GetNameAsString(class UObject* TheObject);
	...
	static void AddToStreamingLevels(class UObject* WorldContextObject, struct FLevelStreamInstanceInfo& LevelInstanceInfo);
	static bool Animation__GetAimOffsets(class AActor* AnimBPOwner, float* Pitch, float* Yaw);
	static bool Animation__GetAimOffsetsFromRotation(class AActor* AnimBPOwner, struct FRotator& TheRotation, float* Pitch, float* Yaw);
	static class FString AppendMultiple(const class FString& A, const class FString& B);
	static bool Array_IsValidIndex(TArray<int32>& TargetArray, int32 Param_Index);
	static void Array_Sort(TArray<int32>& TargetArray, bool bAscendingOrder, class FName VariableName);
	static float Calcs__ClosestPointToSourcePoint(struct FVector& Source, TArray<struct FVector>& OtherPoints, struct FVector* ClosestPoint);
	...
	static bool Capture2D_SaveImage(class ASceneCapture2D* Target, class FString& ImagePath, struct FLinearColor& ClearColour);
	...
	static bool CaptureComponent2D_SaveImage(class USceneCaptureComponent2D* Target, class FString& ImagePath, struct FLinearColor& ClearColour);
	...
	static void Conversions__ColorToString(struct FLinearColor& Color, class FString* ColorAsString);
	static struct FVector Conversions__RotatorToVector(struct FRotator& TheRotator);
	...
	static struct FRotator Conversions__VectorToRotator(struct FVector& TheVector);
	...
	static float DistanceToSurface__DistaceOfPointToMeshSurface(class AStaticMeshActor* TheSMA, struct FVector& TestPoint, struct FVector* ClosestSurfacePoint);
	...
	static void Draw__Thick3DLineFromCharacterSocket(class AActor* TheCharacter, struct FVector& Endpoint, class FName Socket, const struct FLinearColor& LineColor, float Thickness, float Duration);
	static void Draw__Thick3DLineFromSocket(class USkeletalMeshComponent* Mesh, struct FVector& Endpoint, class FName Socket, const struct FLinearColor& LineColor, float Thickness, float Duration);
	...
	static bool FileIO__SaveStringArrayToFile(const class FString& SaveDirectory, const class FString& JoyfulFileName, const TArray<class FString>& SaveText, bool AllowOverWriting);
	static bool FileIO__SaveStringTextToFile(const class FString& SaveDirectory, const class FString& JoyfulFileName, const class FString& SaveText, bool AllowOverWriting);
	...
	static float GetDistanceToCollision(class UPrimitiveComponent* CollisionComponent, struct FVector& Point, struct FVector* ClosestPointOnCollision);
	...
	static int32 GetUnixTimeStamp(struct FDateTime& UTCTime);
	static void GetUserDisplayAdapterBrand(bool* IsAMD, bool* IsNvidia, bool* IsIntel, bool* IsUnknown, int32* UnknownId);
	static void GetUTCFromUnixTimeStamp(int32 UnixTimeStamp, struct FDateTime* UTCTime);
	...
	static bool GetViewportPosition(class UObject* WorldContextObject, struct FVector2D& ScreenPosition, struct FVector2D* OutViewportPosition);
	static bool GetViewportPositionHitResultByChannel(class UObject* WorldContextObject, struct FVector2D& ViewportPosition, ECollisionChannel TraceChannel, bool bTraceComplex, struct FHitResult* OutHitResult);
	static class UWidget* GetWidgetFromName(class UUserWidget* ParentUserWidget, class FName& Param_Name);
	...
	static bool IsPointOnPlane(struct FPlane& Plane, const struct FVector& Point, float Tolerance);
	...
	static void JoyIsKey(struct FKeyEvent& KeyEvent, const struct FKey& Key, bool* Ctrl, bool* Shift, bool* Alt, bool* Cmd, bool* Match);
	...
	static void MaxOfFloatArray(TArray<float>& FloatArray, int32* IndexOfMaxValue, float* MaxValue);
	static void MaxOfIntArray(TArray<int32>& IntArray, int32* IndexOfMaxValue, int32* MaxValue);
	static void MinOfFloatArray(TArray<float>& FloatArray, int32* IndexOfMinValue, float* MinValue);
	static void MinOfIntArray(TArray<int32>& IntArray, int32* IndexOfMinValue, int32* MinValue);
	...
	static bool Physics__LeaveRagDoll(class AActor* TheCharacter, bool SetToFallingMovementMode, float HeightAboveRBMesh, struct FVector& InitLocation, struct FRotator& InitRotation);
	...
	static void PointDistanceToPlane(struct FPlane& Plane, const struct FVector& Point, float* Distance);
	static struct FVector2D ProjectWorldToScreenPosition(struct FVector& WorldLocation);
	...
	static void RemoveFromStreamingLevels(class UObject* WorldContextObject, struct FLevelStreamInstanceInfo& LevelInstanceInfo);
	...
	static bool Text_IsNumeric(class FText& Text);
	static float Text_ToFloat(class FText& Text, bool UseDotForThousands);
	static int32 Text_ToInt(class FText& Text, bool UseDotForThousands);
	static bool TraceData__GetTraceDataFromCharacterSocket(struct FVector* TraceStart, struct FVector* TraceEnd, class AActor* TheCharacter, struct FRotator& TraceRotation, float TraceLength, class FName Socket, bool DrawTraceData, const struct FLinearColor& TraceDataColor, float TraceDataThickness);
	static bool TraceData__GetTraceDataFromSkeletalMeshSocket(struct FVector* TraceStart, struct FVector* TraceEnd, class USkeletalMeshComponent* Mesh, struct FRotator& TraceRotation, float TraceLength, class FName Socket, bool DrawTraceData, const struct FLinearColor& TraceDataColor, float TraceDataThickness);
	static class AActor* Traces__CharacterMeshTrace___ClosestBone(class AActor* TraceOwner, struct FVector& TraceStart, struct FVector& TraceEnd, struct FVector* OutImpactPoint, struct FVector* OutImpactNormal, class FName* ClosestBoneName, struct FVector* ClosestBoneLocation, bool* IsValid);
	static class AActor* Traces__CharacterMeshTrace___ClosestSocket(class UObject* WorldContextObject, class AActor* TraceOwner, struct FVector& TraceStart, struct FVector& TraceEnd, struct FVector* OutImpactPoint, struct FVector* OutImpactNormal, class FName* ClosestSocketName, struct FVector* SocketLocation, bool* IsValid);
	static void UnloadStreamingLevel(class ULevelStreamingDynamic* LevelInstance);
	static void UTCToLocal(struct FDateTime& UTCTime, struct FDateTime* LocalTime);
	...
	static EPathFollowingRequestResult Victory_AI_MoveToWithFilter(class APawn* Pawn, struct FVector& Dest, TSubclassOf<class UNavigationQueryFilter> FilterClass, float AcceptanceRadius, bool bProjectDestinationToNavigation, bool bStopOnOverlap, bool bCanStrafe);
	static bool Victory_Get_Pixel(TArray<struct FLinearColor>& Pixels, int32 ImageHeight, int32 X, int32 Y, struct FLinearColor* FoundColor);
	...
	static bool Victory_SavePixels(const class FString& FullFilePath, int32 Width, int32 Height, TArray<struct FLinearColor>& ImagePixels, bool SaveAsBMP, bool SRGB, class FString* ErrorString);
	...
	static void VictoryCreateProc(int32* ProcessId, const class FString& FullPathOfProgramToRun, const TArray<class FString>& CommandlineArgs, bool Detach, bool Hidden, int32 Priority, const class FString& OptionalWorkingDirectory);
	...
	static struct FVictoryInput VictoryGetVictoryInput(struct FKeyEvent& KeyEvent);
	static struct FVictoryInputAxis VictoryGetVictoryInputAxis(struct FKeyEvent& KeyEvent);
	...
	static bool ViewportPositionDeproject(class UObject* WorldContextObject, struct FVector2D& ViewportPosition, struct FVector* OutWorldOrigin, struct FVector* OutWorldDirection);
